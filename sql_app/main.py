...
@app.post("/users/{user_id}/drinks/", response_model=schemas.Item)
def create_drink_for_user(
    user_id: int, drink: schemas.DrinkCreate, db: Session = Depends(get_db)
):
    return crud.create_user_drink(db=db, drink=drink, user_id=user_id)
